...
abstract class Device {
  ...
  protected async updateChars(refreshDevice: Function) {
    ...
    if(Date.now() - this.lastUpdatedChars > OLD_CHARS_ACCEPTED) await refreshDevice();
    this.lastUpdatedChars = Date.now();
  }

  protected async runLoggedAction(action: Function, log = "action") {
...
abstract class Light extends Device {
  ...
  protected generateDevice() {
    return new YeelightDevice((arg: string) => {if(this.isDebugLogging()) this.platform.log.info(arg);});
  }
...
export class CtMoonLight extends Light {
  ...
  protected generateDevice() {
    return new CtMoonDevice((arg: string) => {if(this.isDebugLogging()) this.platform.log.info(arg);});
  }
}
...
export class ColorLight extends Light {
  ...
  protected generateDevice() {
    return new ColorDevice((arg: string) => {if(this.isDebugLogging()) this.platform.log.info(arg);});
  }
...
export class AirPurifier extends Device {
  ...
  protected device = new AirPurifierDevice((arg: string) => {if(this.isDebugLogging()) this.platform.log.info(arg);});
...
export class RobotVacuum extends Device {
  ...
  protected device = new VacuumDevice((arg: string) => {if(this.isDebugLogging()) this.platform.log.info(arg);});
...
export class MoonSwitch {
  ...
  setOn(value: CharacteristicValue) {
    if(value as boolean == this.nightMode) return;
