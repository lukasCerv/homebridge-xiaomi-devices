...
export class Network extends EventEmitter {
	...
	findDevice(id, rinfo) {
		...
		if(! device && rinfo) {
			...
			if(! device) {
				...
				device = new DeviceInfo(this.debug, this, id, rinfo.address, rinfo.port);
	...
	findDeviceViaAddress(options) {
		...
		if(! device) {
			...
			device = new DeviceInfo(this.debug, this, null, options.address, options.port || PORT);
	...
	createSocket() {
		this._socket  = _createSocket('udp4');
