...
abstract class XiaomiDevice {
    ...
    async getProperties(keys = Object.keys(this.properties)) {
        ...
        if(props) for(let i = 0; i < keys.length; i++) this.properties[keys[i]] = props[i];
    }
...
export class AirPurifierDevice extends XiaomiDevice {
    ...
    async setMode(value: string) {
        if(value != AirPurifierDevice.MODE_IDLE) this.lastMode = value;
    ...
    async setNightMode(mode: boolean) {
        if(this.nightMode == mode) return;
        this.nightMode = mode;
        if(mode && this.properties.mode == AirPurifierDevice.MODE_AUTO) await this.setMode(AirPurifierDevice.MODE_SILENT);
        if(!mode && this.properties.mode == AirPurifierDevice.MODE_SILENT) await this.setMode(AirPurifierDevice.MODE_AUTO);
        if(mode) await this.call('set_led_b', [1]);
        if(!mode) await this.call('set_led_b', [0]);
    }
}
...
export class YeelightDevice extends XiaomiDevice {
    ...
    protected getPowerMode():number {return POWERMODE_NORMAL};

    protected async restoreScene() {}

    async setBrightness(value: number){}
    getBrightness():number {return 0;}

    async setColorTemperature(value: number) {        
        if(!this.getPower()) return;
...
export class CtMoonDevice extends YeelightDevice {
    ...
    async getProperties(keys?: string[]): Promise<void> {
        ...
        if(this.getPower()) this.globalBright = parseInt(this.nightMode ? this.properties.nl_br : this.properties.bright);
    }
    ...
    async setPower(value: boolean): Promise<void> {
        ...
        await sleep(100);
        if(value && this.tempBright) await this.setBrightness(this.tempBright);
        if(value && this.tempCT) await this.setColorTemperature(this.tempCT);
    }
...
export class ColorDevice extends YeelightDevice {
    ...
    async reduceBrightnessRange(limit: number) {
        if(this.reducedRange == limit) return;
    ...
    async setPower(value: boolean): Promise<void> {
        await super.setPower(value);
        await sleep(100);
        if(value && this.tempBright) await this.setBrightness(this.tempBright/this.reducedRange*100);
    }
